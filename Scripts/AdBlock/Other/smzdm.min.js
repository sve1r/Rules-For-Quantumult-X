...
// prettier-ignore
(()=>{let response=null;if(magicJS.isResponse){switch(true){case/^https?:\/\/app-api\.smzdm\.com\/util\/loading/.test(magicJS.request.url):try{let obj=JSON.parse(magicJS.response.body);obj.data.forEach((element)=>{element.start_date='2030-12-24 00:00:00';element.end_date='2030-12-24 23:59:59';element.unix_start_date='1924272000';element.unix_end_date='1924358399';element.is_show_ad='0'});response={body:JSON.stringify(obj)}}catch(err){magicJS.logError(`去除APP启动广告出现异常：${err}`)}break;case/^https?:\/\/homepage-api\.smzdm\.com\/v3\/home/.test(magicJS.request.url):try{let obj=JSON.parse(magicJS.response.body);let component=[];obj.data.component.forEach((element)=>{if(element.zz_type==='banner'){let bannerList=element.zz_content.filter((banner)=>{return banner.tag!=='广告'});element.zz_content=bannerList}else if(element.zz_type==='list'){let contentList=element.zz_content.filter((content)=>{return content.zz_content.model_type!=='ads'});element.zz_content=contentList}else if(element.zz_type==='circular_banner'){element.zz_content.circular_banner_option.background='1';element.zz_content.circular_banner_option.color_card='#ffffff';element.zz_content.circular_banner_option.img=''}if(element.zz_type!=='top_banner'&&element.zz_type!=='hongbao'){component.push(element)}});obj.data.component=component;response={body:JSON.stringify(obj)}}catch(err){magicJS.logError(`首页去广告出现异常：${err}`)}break;case/^https?:\/\/haojia-api\.smzdm\.com\/home\/list/.test(magicJS.request.url):try{let obj=JSON.parse(magicJS.response.body);let bigBanner=obj.data.banner.big_banner.filter((element)=>{return element.ad_banner_id==''});obj.data.banner.big_banner=bigBanner;let rows=obj.data.rows.filter((element)=>{return!element.hasOwnProperty('ad_banner_id')});obj.data.banner.hongbao_banner=[];obj.data.banner.module_banner.hongbao={};obj.data.rows=rows;response={body:JSON.stringify(obj)}}catch(err){magicJS.logError(`好价去广告出现异常：${err}`)}break;case/^https?:\/\/haojia\.m\.smzdm\.com\/detail_modul\/article_releated_modul/.test(magicJS.request.url):try{let obj=JSON.parse(magicJS.response.body);obj.data.lanmu_qikan={};response={body:JSON.stringify(obj)}}catch(err){magicJS.logError(`好价详情页去广告出现异常：${err}`)}break;case/^https?:\/\/baike-api\.smzdm\.com\/home_v3\/list/.test(magicJS.request.url):try{let obj=JSON.parse(magicJS.response.body);obj.data.rows=obj.data.rows.filter((element)=>{return(!element.hasOwnProperty('ad_banner_id')||element.ad_banner_id=='')});response={body:JSON.stringify(obj)}}catch(err){magicJS.logError(`百科去广告出现异常：${err}`)}break;case/^https?:\/\/s-api\.smzdm\.com\/sou\/filter\/tags\/hot_tags/.test(magicJS.request.url):try{let obj=JSON.parse(magicJS.response.body);obj.data.hongbao={};response={body:JSON.stringify(obj)}}catch(err){magicJS.logError(`搜索标签去广告出现异常：${err}`)}break;case/^https?:\/\/s-api\.smzdm\.com\/sou\/list_v10/.test(magicJS.request.url):try{let obj=JSON.parse(magicJS.response.body);obj.data.rows=obj.data.rows.filter((element)=>{return element.article_tag!=='广告'});response={body:JSON.stringify(obj)}}catch(err){magicJS.logError(`搜索结果去广告出现异常：${err}`)}break;case/^https?:\/\/zhiyou\.m\.smzdm\.com\/user\/vip\/ajax_get_banner/.test(magicJS.request.url):try{let obj=JSON.parse(magicJS.response.body);obj.data.big_banner=obj.data.big_banner.filter((element)=>{return element.logo_title!=='广告'});response={body:JSON.stringify(obj)}}catch(err){magicJS.logError(`值会员权益中心banner去广告出现异常：${err}`)}break;default:magicJS.notify('触发意外的请求处理，请确认脚本或复写配置正常。');break}}else{magicJS.notify('触发意外的请求处理，请确认脚本或复写配置正常。')}if(response){magicJS.done(response)}else{magicJS.done()}})();
