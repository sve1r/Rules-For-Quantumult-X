...
async function _checkPaidContentByCloud(filteredUrls, newData) {
    ...
    if (!zheyeServer) {
        ...
        $.logger.debug(`服务端地址\n${url}`);
        await $.http.post({url: url, headers: {"Content-Type": "application/json"}, body: filteredUrls}).then(resp => {
    ...
    function _requestAnswerContent(i) {
        return new Promise(resolve => {
            ...
            if (!url || url === "" || !url.startsWith("https://www.zhihu.com/appview/v2/answer")) {
                ...
                $.http.get({url: filteredUrls[i], timeout: 1e3}).then(resp => {
        ...
        const _adaptOpts = _opts => {
            try {
                ...
                if (typeof _opts === "string") {
                    if (env.isLoon) newOpts = {openUrl: _opts}; else if (env.isQuanX) newOpts = {"open-url": _opts}; else if (env.isSurge) newOpts = {url: _opts}
