 ...
 * 处理非 SDK 广告
 */
function handleNoneSdkAd(content) {
  let obj = JSON.parse(content);
  if (true === url.includes('/2/cardlist')) {
    handleCardList(obj); //卡片列表
  } else if (true === url.includes('/2/checkin/show')) {
    handleCheckinShow(obj); //签到显示
  } else if (true === url.includes('/2/client/publisher_list')) {
    handleIndexPageButton(obj); // 首页右上角按钮
  } else if (true === url.includes('/2/comments/build_comments')) {
    handleBuildComments(obj); //评论区构建
  } else if (true === url.includes('/2/container/asyn')) {
    handleContainerAsyn(obj); //异步容器
  } else if (true === url.includes('/2/direct_messages/user_list')) {
    handleDirectMessages(obj);
  } else if (true === url.includes('/2/flowlist')) {
    handleFlowList(obj);
  } else if (true === url.includes('/2/flowpage')) {
    handleFlowPage(obj);
  } else if (true === url.includes('/2/groups/allgroups/v2')) {
    handleGroupsAllGroups(obj);
  } else if (true === url.includes('/2/messageflow/notice')) {
    handleMessageFlow(obj);
  } else if (true === url.includes('/2/page')) {
    handlePage(obj);
  } else if (true === url.includes('/2/profile/container_timeline')) {
    handleProfileContainerTimeline(obj);
  } else if (
    true === url.includes('/2/profile/dealatt') ||
    url.includes('/2/friendships/')
  ) {
    handleProfileDealAtt(obj);
  } else if (true === url.includes('/2/profile/me')) {
    handleProfileMe(obj);
  } else if (true === url.includes('/2/profile/statuses/tab')) {
    handleProfileStatusesTab(obj);
  } else if (true === url.includes('/2/profile/userinfo')) {
    handleProfileUserInfo(obj);
  } else if (true === url.includes('/2/push/active')) {
    handlePushActive(obj);
  } else if (true === url.includes('/2/search/')) {
    handleSearch(obj);
  } else if (true === url.includes('/2/searchall')) {
    handleSearchAll(obj);
  } else if (
    true === url.includes('/2/shproxy/chaohua/discovery/searchactive')
  ) {
    handleChaohuaSearch(obj);
  } else if (true === url.includes('/2/statuses/container_detail?')) {
    handleContainerDetail(obj);
  } else if (true === url.includes('/2/statuses/container_detail_comment')) {
    handleContainerDetailComment(obj);
  } else if (
    true === url.includes('/2/statuses/container_timeline_hot') ||
    url.includes('/2/statuses/unread_hot_timeline')
  ) {
    handleContainerTimelineHot(obj);
  } else if (
    true === url.includes('/2/statuses/container_timeline?') ||
    url.includes('/2/statuses/container_timeline_unread')
  ) {
    handleContainerTimeline(obj);
  } else if (true === url.includes('/2/statuses/container_timeline_topic')) {
    handleContainerTopicTimeline(obj);
  } else if (true === url.includes('/2/statuses/extend')) {
    handleContainerExtend(obj);
  } else if (true === url.includes('/2/statuses/repost_timeline')) {
    handleRepostTimeline(obj);
  } else if (true === url.includes('/2/statuses/show')) {
    handleStatusesShow(obj);
  } else if (true === url.includes('/2/video/full_screen_stream')) {
    handleFullScreenVideoStream(obj); //全屏视频流
  } else if (true === url.includes('/2/video/tiny_stream_mid_detail')) {
    handleVideoStreamDetail(obj); //视频中的详情
  } else if (true === url.includes('/2/video/tiny_stream_video_list')) {
    handleVideoStreamList(obj);
  } else if (
    true === url.includes('/2/!/huati/discovery_home_bottom_channels')
  ) {
    handleTopicDiscovery(obj); //话题发现页面
  } else if (true === url.includes('/aj/appicon/list')) {
    handleAppIconList(obj); //app 图标
  } else if (
    true === url.includes('/v1/ad/preload') ||
    url.includes('/v2/ad/preload')
  ) {
    handleAdPreload(obj); //预加载广告
  } else if (
    true === url.includes('/wbapplua/wbpullad.lua') ||
...
function handleIndexPageButton(obj) {
  if (obj?.elements?.length > 0) {
    obj.elements = obj.elements.filter((i) =>
      ['写微博', '相册', '图片', '视频', '签到/点评']?.includes(i?.app_name),
    );
  }
}
...
function handleProfileContainerTimeline(obj) {
  ...
  if (obj?.items?.length > 0) {
    ...
    for (let item of obj.items) {
      if (
        item?.data?.left_hint?.[0]?.content === '全部微博(0)' &&
        item?.data?.card_type === 216
      ) {
        // 全部微博为0的博主
        break;
      } else if (
        /内容/.test(item?.data?.name) &&
        item?.data?.card_type === 58
      ) {
        // 个人微博页刷完后的推荐微博
        break;
      } else {
  ...
  if (url.includes('container_timeline')) {
    ...
    if (obj?.items?.length > 0) {
      ...
      for (let item of obj.items) {
        if (item?.category === 'feed') {
          ...
          if (
            [19, 22, 118, 206, 208, 217, 236, 249, 261]?.includes(
              item?.data?.card_type,
            )
          ) {
            continue;
          ...
          if (item?.item_category === 'insert_item') {
            ...
            if (item?.items?.length > 0) {
              ...
              for (let ii of item.items) {
                ...
                if (
                  [118, 182, 192, 217, 247, 264]?.includes(ii?.data?.card_type)
                ) {
                  continue;
    ...
    if (obj?.channelInfo?.channels?.length > 0) {
      ...
      for (let channel of obj.channelInfo.channels) {
        // 顶部标签栏 白名单
        if (
          ['band_channel', 'discover_channel', 'trends_channel']?.includes(
            channel?.key,
          )
        ) {
                ...
                if (item?.category === 'feed') {
                  ...
                  if (item?.items?.length > 0) {
                    ...
                    for (let ii of item.items) {
                      ...
                      if (
                        [118, 182, 192, 217, 247, 264]?.includes(
                          ii?.data?.card_type,
                        )
                      ) {
                        continue;
      ...
      for (let item of obj.header.data.items) {
        if (item?.category === 'card') {
          ...
          if (item?.items?.length > 0) {
            ...
            for (let i of item.items) {
              if (
                [118, 182, 192, 217, 247, 264]?.includes(i?.data?.card_type)
              ) {
                continue;
              } else {
                newII.push(i);
...
function handleSearchAll(obj) {
  ...
  if (obj?.footer) {
    ...
    if (obj?.footer?.data?.menus?.length > 0) {
      // 底部菜单
      obj.footer.data.menus = obj.footer.data.menus.filter(
        (i) => !/\d+_ai\./.test(i?.pic),
      );
    }
  }
            ...
            for (let ii of item.items) {
              if (ii?.cate_id === '638' && ii?.readtimetype === 'card') {
                ...
                if (!isAd(ii?.data)) {
                  ...
                  if (
                    [3, 17, 22, 30, 42, 89, 101, 206]?.includes(
                      ii?.data?.card_type,
                    )
                  ) {
                    continue;
    ...
    obj.pageHeader.data.items = obj.pageHeader.data.items.filter((item) => {
      return !(
        ...
        (item?.data?.itemid === 'top_searching' &&
          item?.data?.card_type === 248) // 底部大家都在搜
      );
    });
  }
      ...
      if (!isAd(item?.data) && !isAd(item?.status)) {
        if (item?.category === 'dynamic') {
          ...
          if (item?.status?.screen_name_suffix_new?.length > 0) {
            if (
              item?.status?.screen_name_suffix_new?.[3]?.content === '快转了'
            ) {
              continue;
            }
          }
    ...
    for (let item of obj.items) {
      if (item?.category === 'feed') {
        ...
        if ([4, 197, 1012]?.includes(item?.data?.card_type)) {
          ...
          if (
            item?.data?.card_type === 31 &&
            item?.data?.hotwords?.length > 0
          ) {
            // 31 搜索框滚动热词
            item.data.hotwords = [];
          }
          if (
            ...
            item?.data?.hasOwnProperty('card_ad_style')
          ) {
            continue;
          }
          newItems.push(item);
...
function handleWbPullAd(obj) {
  log(`ℹ️ 脚本处理拉取广告`);
  log('obj:', obj);
  log('cached_ad:', obj?.cached_ad);
  log('ads', obj?.cached_ad?.ads);
